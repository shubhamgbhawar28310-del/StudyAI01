-- ============================================================================
-- TASK & PLANNER INTEGRATION MIGRATION (FIXED VERSION)
-- ============================================================================
...
-- FIXED: Handles TEXT to UUID type conversion for task_id
-- ============================================================================

-- Step 1: Check and fix task_id column type
-- ============================================================================

-- Check if task_id is TEXT and convert to UUID if needed
DO $$
BEGIN
  -- Check the data type of task_id
  IF EXISTS (
    SELECT 1 
    ...
    RAISE NOTICE 'Converting task_id from TEXT to UUID...';
    ...
    RAISE NOTICE '✅ task_id converted to UUID';
  ELSE
    RAISE NOTICE '✅ task_id is already UUID type';
  END IF;
END $$;

-- Step 2: Add missing columns to schedule_events table
-- ============================================================================
...
-- Step 3: Add status column to tasks table if missing
-- ============================================================================
...
-- Step 4: Clean up orphaned task_id references
-- ============================================================================

-- Now that task_id is UUID, we can safely clean up orphaned references
UPDATE schedule_events
SET task_id = NULL
WHERE task_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM tasks WHERE id = schedule_events.task_id);
...
-- Step 7: Create unified view for task-schedule data
-- ============================================================================

DROP VIEW IF EXISTS task_schedule_view;

CREATE VIEW task_schedule_view AS
SELECT 
  t.id as task_id,
  t.user_id,
...
-- Step 8: Create helper function to sync task and event status
-- ============================================================================
...
-- Step 9: Create trigger for automatic status sync
-- ============================================================================
...
-- Step 10: Create function to get unscheduled tasks
-- ============================================================================
...
-- Step 11: Create function to get available time slots
-- ============================================================================
...
-- Step 13: Create materialized view for dashboard stats (optional optimization)
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS user_productivity_stats;
...
BEGIN
  ...
  RAISE NOTICE '   - task_id column type: UUID';
  RAISE NOTICE '   - Foreign key: schedule_events.task_id → tasks.id';
  RAISE NOTICE '   - Status tracking: enabled';
  RAISE NOTICE '   - Auto-sync trigger: active';
